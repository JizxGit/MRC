...
tf.flags.DEFINE_string("experiment_name", "exp_" + time.strftime("%Y%m%d_%H%M%S", time.localtime()),
...
MODEL_DIR = os.path.join(ROOT_DIR, "model", config.experiment_name)  # 模型保存根目录
CHECK_POINT_DIR = os.path.join(MODEL_DIR, "checkpoint")  # 模型保存目录
BEST_MODEL_DIR = os.path.join(MODEL_DIR, "best_checkpoint")  # 最好的模型保存目录
SUMMARY_DIR = os.path.join(ROOT_DIR, "summary", config.experiment_name)  # tensorboard 目录
# 创建tensorboard文件夹
